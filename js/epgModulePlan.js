...
function intoModulePlan(){
	...
	/*功能页*/
	/*
	this.dramaesArray=document.getElementsByName("dramaes");//追剧
	this.histroyArray=document.getElementsByName("histroy");//观看历史
	this.collectArray=document.getElementsByName("collect");//收藏
	this.installArray=document.getElementsByName("install");//三屏设置
	this.interactArray=document.getElementsByName("interact");//设置
	*/
	...
	if(newestArray!=null){
		...
		if(newOnlinePrograms){
			this.getNewestInfo(newOnlinePrograms);
		}else{
			parent.loadNewOnlineProgramsFun(function(data){
				if(data!=null)getNewestInfo(data);
			});
		}
	}
	if(playArray!=null){
...
function functionModulePlan(name,pid){
	...
	switch (name) {
		...
		case "newest":
				tvsee.pageWidgets.getByName("widgetMenu").hideWin();
				gotoPageUrl("recentlyUpdatedList.html?windowSize=fullScreen");
				break;
		case "URL":
...
// 获得最近更新信息
function getNewestInfo(newOnlinePrograms){
...
function loadNewOnlineProgramsFun(callback){
	var url = getAjaxUrl("getNewOnlinePrograms",20);
	console.log("最近更新信息"+url);
